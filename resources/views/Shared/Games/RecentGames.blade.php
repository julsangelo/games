<div class="mt-4 mb-4">
    <h1 class="text-2xl text-[#ED9121] font-bold">Recent Games</h1>
    <div class="grid grid-cols-5 gap-3 mt-3">
        @foreach ($recent as $games)
            <a href={{$games->url}} target="_blank" class="grid col-span-1 bg-gray-50 shadow rounded-xl hover:bg-[#ED9121] hover:text-white relative">
                ...
                <h1 class="bg-[#9DC183] absolute py-1 px-2 font-semibold rounded-br-xl rounded-tl-lg">{{$games->rank}}</h1>
                @if ($games->isNew == true)
